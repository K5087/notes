...
- 下载并安装[MSBuild工具链](https://visualstudio.microsoft.com/downloads/#build-tools-for-visual-studio-2026)

## 编译命令

UE自带一套编译工具，该工具分为UBT和UHT，UHT负责分析头文件中的宏来生成代码。UBT会先调用UHT,然后再使用编译器编译C++代码。这些工具可以在`Engine/Build/BatchFiles/`中找到。

在开始菜单中可以找到`Visual Studio Developer Command Prompt`和`Visual Studio Developer PowerShell`两个快捷方式来打开使用msbuild工具链的环境。
